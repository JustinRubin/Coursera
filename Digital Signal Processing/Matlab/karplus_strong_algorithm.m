...
% Parameters:
    ...
    % duration: duration of the chord (sec)
    % alpha:    attenuation in K-S algorithm
...
% This is used to correct alpha later, so that all notes decay together (with the same decay rate)
...
for i = 1:length(F0)
    ...
    % Correct current alpha so that all the notes decay together (with the same decay rate)
    ...
    % Construct the chord by adding the generated note (with the appropriate gain)
